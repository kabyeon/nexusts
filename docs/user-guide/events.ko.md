...
NexusJS는 `@kabyeon/nexusjs/events` 모듈 아래 `@nestjs/event-emitter`와 AdonisJS의 emitter를 본떠 만든 이벤트 시스템을 제공한다.
...
events 모듈은 **`@kabyeon/nexusjs/core`와 분리**되어 있으며 자체 번들 진입점으로 제공된다.
...
import { EventService, scanForListeners } from '@kabyeon/nexusjs/events';
