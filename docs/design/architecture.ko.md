...
| 서브 경로 | 용도 |
| -------- | ------- |
| `@kabyeon/nexusjs/view` | 뷰 엔진 (기본 `RenduAdapter`) |
| `@kabyeon/nexusjs/view/inertia` | Inertia 어댑터 + 헬퍼 |
| `@kabyeon/nexusjs/view/inertia/ssr` | React/Vue/Svelte/Solid SSR 어댑터 |
| `@kabyeon/nexusjs/orm` | ORM 어댑터 (현재 Drizzle) |
| `@kabyeon/nexusjs/runtime` | 런타임 어댑터 |

공개 진입점(`@kabyeon/nexusjs`)은 안정적이고 합의된 표면만 다시 export합니다. 그 외는 모두 **고급**이며 메이저 버전 bump 없이 변경될 수 있습니다.
...
| 공개 표면은 **작게 유지** | 실험적인 것은 `@kabyeon/nexusjs/<x>` 서브 경로 뒤로. |
