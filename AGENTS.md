# AGENTS.md — NexusJS Contributor & Module-Author Guide

> A working guide for AI agents (and humans) developing, testing, and
> extending the NexusJS framework. The conventions here were derived
...
`@kabyeon/nexusjs` is a Bun-native fullstack framework. It
publishes **30 independent modules** (one entry point each under
`@kabyeon/nexusjs/*`), so a user only ships the code they actually
...
```
nexusjs/
├── src/                       # Framework source (30 modules)
│   ├── core/                  # Always-shipped core (DI, decorators, http, validation, view, runtime)
│   ├── cli/                   # `nx` command runner (scaffolding, make:*, db:*)
│   ├── view/inertia/          # Inertia.js v2 server-side adapter (lazy-loaded)
│   ├── auth, cache, config, crypto, drive, drizzle, events, graphql,
│   │   grpc, health, i18n, limiter, logger, mail, metrics, openapi,
│   │   queue, redis, resilience, schedule, session, shield, sse,
│   │   static, tracing, upload, view, ws/
│   └── index.ts               # `@kabyeon/nexusjs` (core) public entry
├── tests/                     # Vitest suites
...
│   └── analysis/              # NestJS / AdonisJS gap analysis (en + .ko)
├── scripts/
│   └── clean-examples.sh      # Kills leftover example processes + port 3000
├── build.ts                   # Bun.build → tsc --emitDeclarationOnly → flatten dist/src/* → dist/*
├── package.json               # `name: "@kabyeon/nexusjs"`, exports map, bin, peer-deps
├── tsconfig.json              # Root config (uses experimentalDecorators + useDefineForClassFields:false)
├── tsconfig.build.json        # Build-time include list (one glob per module)
├── vitest.config.ts           # Vitest with @core/@app aliases
└── examples/                  # Each example's tsconfig.json is generated by the smoke test
```
...
This is the exact flow that produced `@kabyeon/nexusjs/graphql` and
`@kabyeon/nexusjs/resilience` in v0.7.0. Follow it in order.
...
```bash
mkdir -p src/<name>/decorators tests/<name>
```
...
async function loadPeer() {
  ...
  try {
    ...
    throw new Error("[nexusjs/<name>] Install `<peer-dep>` with `bun add <peer-dep>`.");
  }
}
```
...
1. **`Bun.build()`** — bundles each `entrypoints:` entry into
   `dist/src/<name>/index.js`. Per-module entry means each
   `<name>` ships as its own JS file.
2. **`tsc --emitDeclarationOnly`** — emits `dist/src/<name>/index.d.ts`
...
beforeAll(async () => {
  const mod = await import("@kabyeon/nexusjs/<name>");
...
The dynamic `import("@kabyeon/nexusjs/<name>")` pattern is critical —
...
export async function loadPeer() {
  if (_peer) return _peer;
  if (_attempted) {
    throw new Error("[nexusjs/<name>] `<peer-dep>` failed to load. Install with `bun add <peer-dep>`.");
  }
  _attempted = true;
  try {
    ...
    throw new Error("[nexusjs/<name>] `<peer-dep>` is required. Install with `bun add <peer-dep>`.\nOriginal error: " + (err as Error).message);
  }
}
```
...
every user of the framework. `@kabyeon/nexusjs/graphql` users need
...
```
feat(graphql): add @kabyeon/nexusjs/graphql module with v0.6.9
feat(resilience): add @kabyeon/nexusjs/resilience module (retry + circuit + bulkhead) v0.7.0
...
### `Cannot find module '@kabyeon/nexusjs/<name>'`
...
### `[nexusjs/<name>] <peer-dep> failed to load`
...
- **"Add a new ORM"** — Drizzle is the default and the only
  ...
  `optional` peer-dep path (`@kabyeon/nexusjs/drizzle` is a peer).
